    ...
    def generate_qa(self):
        ...
        for i, image_info in enumerate(tqdm(self.image_info_list)):
            if (Path(self.output_path) / "qa_info" / f"qa_{i}.json").exists():
                continue
            sys_prompt, user_prompt = eval(image_info["source"]).generate_prompt(image_info, self.dataset_info_dict[image_info["source"]])
...
def parse_arguments():
    ...
    parser.add_argument('--task_name', type=str, default="national_flag_recognition", help='The name of the target dataet')
