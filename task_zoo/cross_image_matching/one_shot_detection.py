...
class fss_1000(BaseDataset):
    DATA_METAINFO = {
        "image_path": "/mnt/petrelfs/share_data/yingkaining/lvlm_evaluation/data_process/data/perdet/fss_1000/FSS-1000",
        "sampling_num": 100,
...
class DatasetPACOPart(Dataset):
    def __init__(self, datapath, fold=1, transform=None, split='val', shot=1, use_original_imgsize=False, box_crop=True):
        ...
        self.shot = shot
        self.img_path = "/mnt/petrelfs/share_data/yingkaining/lvlm_evaluation/data_process/data/coco"
...
class paco_part(BaseDataset):
    DATA_METAINFO = {
        "anno_path": "/mnt/petrelfs/share_data/yingkaining/lvlm_evaluation/data_process/data/perdet/paco_part/paco/paco_part_val.pkl",
        "save_image_path": "/mnt/petrelfs/share_data/yingkaining/lvlm_evaluation/data_process/taskonomy_evaluation_data/cross_image_matching/one_shot_detection/images",
        "image_path": "/mnt/petrelfs/share_data/yingkaining/lvlm_evaluation/data_process/data/coco/val2017",
        "sampling_num": 100,
        "visual_input_component": ["natural_image",],
        "dataset_description": "xxx"
    }
    ...
    def parse_images_info(self):
        self.images_info = list()
        paco_part_data_info = DatasetPACOPart(datapath="/mnt/petrelfs/share_data/yingkaining/lvlm_evaluation/data_process/data/perdet")
        i = 0
        # anno_info_lsit = mmcv.load(self.anno_path)
        bar = tqdm(400)
        for sample_info in paco_part_data_info:
            ...
            self.save_rgb_image(query_img.permute(1, 2, 0).numpy()*255, query_image_path)
            self.save_rgb_image(support_img.permute(1, 2, 0).numpy()*255, support_image_path)
